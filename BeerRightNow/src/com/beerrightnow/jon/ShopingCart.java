...
public class ShopingCart extends SherlockActivity {
			...
			public void onClick(View v) {
				...
				if(notifi.length()>0)
				{
					...
					for (int i = 0; i < notifi.length(); i++) {
					 ...
					 AlertDialog alert= new AlertDialog.Builder(ShopingCart.this)
						.setMessage(value)
					...
					//Toast.makeText(ShopingCart.this, value, Toast.LENGTH_SHORT).show();
				}
				else
				{
	...
	private void addToView() {
		...
		if (!cartResponse.getBeer().getProducts().isEmpty()) {
			...
			CartInfo.getInstance().setBeerCount(String.valueOf(cartResponse.getBeer().getProducts().size()));
		...
		{
			CartInfo.getInstance().setBeerCount(String.valueOf("0"));
		}
		if (!cartResponse.getLiquor().getProducts().isEmpty()) {

			
			CartInfo.getInstance().setLiquorCount(String.valueOf(cartResponse.getLiquor().getProducts().size()));
			TextView headView = new TextView(this);
		...
		{
			CartInfo.getInstance().setLiquorCount(String.valueOf("0"));
		}
