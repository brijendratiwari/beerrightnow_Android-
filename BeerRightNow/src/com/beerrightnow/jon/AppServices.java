...
import static com.beerrightnow.jon.Settings.API_URL;
import static com.beerrightnow.jon.Settings.APP;
import static com.beerrightnow.jon.Settings.NOTIFIER;
import static com.beerrightnow.jon.Settings.ORG;
import static com.beerrightnow.jon.Settings.PASSWORD;
import static com.beerrightnow.jon.Settings.USER;
import static com.beerrightnow.utility.Utils.TAG;
import static com.beerrightnow.utility.Utils.displayMessage;
...
public final class AppServices {
	...
	static synchronized ApigeeDataClient getClient(Context context) {
		if (client == null) {
			if (ORG.equals("<<your org name here>>")) {
				...
				Log.e(TAG, "ORG value");
				ApigeeClient apigeeClient = new ApigeeClient(ORG, APP, API_URL,
						context);
	...
	static void loginAndRegisterForPush(final Context context) {
		...
		if ((USER != null) && (USER.length() > 0)) {
			...
			if (dataClient != null) {
				dataClient.authorizeAppUserAsync(USER, PASSWORD,
				...
				Log.e(TAG,
						"Data client is null, did you set ORG value in Settings.java?");
			}
		} else {
			registerPush(context);
		}
	...
	static void register(final Context context, final String regId) {
		...
		if (dataClient != null) {
			...
			dataClient.registerDeviceForPushAsync(
					dataClient.getUniqueDeviceID(), NOTIFIER, regId, null,
					new DeviceRegistrationCallback() {
						...
						public void onResponse(Device device) {
							...
							displayMessage(context, "Device registered as: "
									+ regId);
							ApigeeDataClient dataClient = getClient(context);
							...
							if (dataClient != null) {
								...
								// connect Device to current User - if there is
								// one
								...
								if (dataClient.getLoggedInUser() != null) {
									...
									dataClient.connectEntitiesAsync("users",
											dataClient.getLoggedInUser()
													.getUuid().toString(),
											"devices", device.getUuid()
													.toString(),
											new ApiResponseCallback() {
												@Override
	...
	static void sendMyselfANotification(final Context context) {
		if (device == null) {
			displayMessage(context,
					"Device not registered. ORG value set in Settings.java?");
		} else {
			ApigeeDataClient dataClient = getClient(context);
			if (dataClient != null) {
				GCMDestination destination = GCMDestination
						.destinationSingleDevice(device.getUuid());
				GCMPayload payload = new GCMPayload();
				payload.setAlertText("Hi there!");

				dataClient.pushNotificationAsync(payload, destination,
						NOTIFIER, new ApiResponseCallback() {
